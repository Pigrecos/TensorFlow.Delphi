...
type
  ...
  public
    ...
    procedure Free;
    // properties
    property Nodes: TsgRecordList<TNodeDef> read FNodes;
...
procedure TGraphDef.Init;
begin
  Self := Default(TGraphDef);
  FNodes := TsgRecordList<TNodeDef>.From(nil);
end;

procedure TGraphDef.Free;
begin
  FNodes.Free;
end;
    ...
    case fieldNumber of
      ...
      TGraphDef.ftVersions:
        begin
          ...
          Pb.Push;
          try
            var v : TVersionDef := Value.FVersions;
            LoadVersionDef(v);
            Value.FVersions := v;
          finally
            Pb.Pop;
      ...
      TGraphDef.ftLibrary:
        begin
          ...
          Pb.Push;
          try
            var v : TFunctionDefLibrary := Value.FLibrary;
...
begin
  if Value.FNodes.Count > 0 then
    S.SaveList<TNodeDef>(Value.FNodes, SaveNodeDef, TGraphDef.ftNodes);
  S.SaveObj<TVersionDef>(Value.FVersions, SaveVersionDef, TGraphDef.ftVersions);
  S.Pb.writeInt32(TGraphDef.ftVersion, Value.Version);
  S.SaveObj<TFunctionDefLibrary>(Value.FLibrary, SaveFunctionDefLibrary, TGraphDef.ftLibrary);
end;
