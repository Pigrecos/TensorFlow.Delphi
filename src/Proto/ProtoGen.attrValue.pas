...
type
  ...
  const
    ...
    ftFuncs = 9;
  private
    FSs: TsgRecordList<TBytes>;
    FIs: TsgRecordList<Int64>;
    FFs: TsgRecordList<Single>;
    FBs: TsgRecordList<Boolean>;
    FTypes: TsgRecordList<TDataType>;
    FShapes: TsgRecordList<TTensorShapeProto>;
    FTensors: TsgRecordList<TTensorProto>;
    FFuncs: TsgRecordList<TNameAttrList>;
  public
    procedure Init;
    procedure Free;
    // properties
    property Ss: TsgRecordList<TBytes> read FSs;
    property &Is: TsgRecordList<Int64> read FIs;
    property Fs: TsgRecordList<Single> read FFs;
    property Bs: TsgRecordList<Boolean> read FBs;
    property Types: TsgRecordList<TDataType> read FTypes;
    property Shapes: TsgRecordList<TTensorShapeProto> read FShapes;
    property Tensors: TsgRecordList<TTensorProto> read FTensors;
    property Funcs: TsgRecordList<TNameAttrList> read FFuncs;
  end;
...
procedure TListValue.Init;
begin
  Self := Default(TListValue);

  var m : TsgItemMeta;
  m.Init<TBytes>;
  FSs := TsgRecordList<TBytes>.From(@m);

  m := default(TsgItemMeta);
  m.Init<Int64>;
  FIs := TsgRecordList<Int64>.From(@m);

  m := default(TsgItemMeta);
  m.Init<Single>;
  FFs := TsgRecordList<Single>.From(@m);

  m := default(TsgItemMeta);
  m.Init<Boolean>;
  FBs := TsgRecordList<Boolean>.From(@m);

  m := default(TsgItemMeta);
  m.Init<TDataType>;
  FTypes := TsgRecordList<TDataType>.From(@m);

  m := default(TsgItemMeta);
  m.Init<TTensorShapeProto>;
  FShapes := TsgRecordList<TTensorShapeProto>.From(@m);

  m := default(TsgItemMeta);
  m.Init<TTensorProto>;
  FTensors := TsgRecordList<TTensorProto>.From(@m);

  m := default(TsgItemMeta);
  m.Init<TNameAttrList>;
  FFuncs := TsgRecordList<TNameAttrList>.From(@m);
end;

procedure TListValue.Free;
begin
  FSs.Free;
  FIs.Free;
  FFs.Free;
...
procedure TAttrValue.Init;
begin
  Self := Default(TAttrValue);
end;

procedure TAttrValue.Free;
begin
end;

{ TNameAttrList }

Constructor TNameAttrList.Init;
begin
  inherited Create;
  FAttr := TMultiMap<string, TAttrValue>.Create;
end;

destructor TNameAttrList.Free;
begin
  FAttr.Free;
  inherited Destroy;
end;
...
begin
  ...
  try
    for i := 0 to Value.Ss.Count - 1 do
      h.Pb.writeRawBytes(Value.FSs[i]^);
  ...
  try
    for i := 0 to Value.&Is.Count - 1 do
      h.Pb.writeRawVarint64(Value.FIs[i]^);
  ...
  try
    for i := 0 to Value.Fs.Count - 1 do
      h.Pb.writeRawData(Value.FFs[i], sizeof(Single));
  ...
  try
    for i := 0 to Value.Bs.Count - 1 do
      h.Pb.writeRawVarint32(Integer(Value.FBs[i]^));
  ...
  if Value.FShapes.Count > 0 then
    S.SaveList<TTensorShapeProto>(Value.FShapes, SaveTensorShapeProto, TListValue.ftShapes);
  if Value.FTensors.Count > 0 then
    S.SaveList<TTensorProto>(Value.FTensors, SaveTensorProto, TListValue.ftTensors);
  if Value.FFuncs.Count > 0 then
    S.SaveList<TNameAttrList>(Value.FFuncs, SaveNameAttrList, TListValue.ftFuncs);
end;
